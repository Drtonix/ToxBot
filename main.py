		...
		async def on_member_join(member):
			...
			for ch in bot.get_guild(member.guild.id).channels:
				if ch.name == "💬┃био-отходняк-чат": 
					await bot.get_channel(ch.id).send(f'Поздоровайтесь с новым участником БДБ, {member}!')

		@bot.event
		async def on_member_remove(member):
			await member.send('Жаль что вы уходите с сервера, надеюсь вам было весело.')
			for ch in bot.get_guild(member.guild.id).channels:
				if ch.name == "💬┃био-отходняк-чат":
					await bot.get_channel(ch.id).send(f'К сожалению, пользователь {member} покинул нас.')
...
Спасибо что пользуетесь ToxBot!''')
		...
		async def on_command_error(ctx, error):
			if isinstance(error, commands.CommandNotFound):
				await ctx.send(embed = discord.Embed(description = f'{ctx.author.name}, данной команды не существует.', color = discord.Colour.from_rgb(230,0,0)))
		...
		async def cal(ctx, *, expression:str):
			try:
				...
				await ctx.send("Выражение не имеет решения.")
